...
<div class="card mb-3">
    ...
    by xample: <a class="text-decoration-none" href="/home?authors={{ $post[0]->user->name }}"> {{ $post[0]->user->name }}</a> in 
    <a class="text-decoration-none" href="/home?kategori={{ $post[0]->category->slug }}">{{ $post[0]->category->title }}</a> {{ $post[0]->created_at->diffForHumans() }}
...
<div class="container mb-3">
    <div class="row">
            ...
            <div class="col-md-4 mb-3">
                <div class="card">
                    <div class="text-white bg-dark px-3 py-2 position-absolute"><a class="text-decoration-none text-white" href="/home?kategori={{ $res->category->slug }}">{{ $res->category->title }}</a></div>
                    <img class="card-img-top" src="{{ asset('storage/' . $res->image) }}" alt="{{ $res->category->title }}">
                    <div class="card-body">
                        <h5 class="card-title"><a class="text-decoration-none text-dark" href="/feature/{{ $res->slug }}">{{ $res->title }}</a></h5>
                        <p>
                            <small class="text-muted">
                                by. <a class="tex-decoration-none" href="/home?authors={{ $res->user->name }}">
